...
mainSite = "https://www.nbcnews.com/?refresh=1/"
...
filenameArticle = 'articles_nbc.json'
...
for i in range(1, n//9+1):
    ...
    for articles in soup.find_all('h2', class_="tease-card__headline"):
        # find the link to the next article.
        # Must give header to cbs sports
        articleLink =  articles.a['href']
        if ("nbcnews.com" in articleLink):
            ...
            title = articleSoup.find('h1', class_="article-hero-headline__htag")
            if(not title == None):
            ...
            subheader = articleSoup.find('div', class_="article-dek")
            ...
            # author
            author = articleSoup.find('div', class_="article-body__byline")
            if(not author == None):
        ...
        # time posted
        posted = articleSoup.find('time', class_="relative z-1")
        if(posted == None):
            article['date'] = None
            article['time'] = None
        else:
            date_time = posted.text.split(",")
            date, time = ", ".join(date_time[:2]), " ".join(date_time[2:])
            
            article['date'] = date
            article['time'] = time
#         # content
            content = articleSoup.find('div', class_="article-body__content")
            # sections
            sections = []
